// MatchView.jsx — one assignment beat at a time: event card → your call →
// feedback. Solo, so it's always your turn. Alongside the choices, the
// Newsroom Panel shows your corkboard filling in, one pinned card per
// assignment filed, and the eleven years passing — the build-status view that
// replaces a map (spec §1, §3).

import { useEffect, useState } from 'react';
import { emitAck, errorText } from '../../services/socket.js';
import { Art } from '../../services/assets.jsx';
import NewsroomPanel from '../shared/NewsroomPanel.jsx';
import MetersBar from '../shared/MetersBar.jsx';

const reporterScore = (m) => (m ? (m.truth || 0) + (m.sources || 0) + (m.standing || 0) : 0);
...
export default function MatchView({ state, dispatch }) {
  ...
  const standingLow = (match.meters?.standing ?? 50) <= 15;
  const otherLow = Object.entries(match.meters || {}).find(([k, v]) => k !== 'standing' && v <= 15);

  return (
    <div className="match">
      <header className="match-header">
        <div className="nation-chip reporter">📰 <b>Reconstruction Reporter</b></div>
        <div className="trail-chip" title="Everyone plays the same reporter">The Galveston Reporter</div>
        <div className="hold-chip" title="Your three meters added up (max 300)">
          Story Score <b>{reporterScore(match.meters)}</b><span className="muted"> / 300</span>
        </div>
        {phase && (
          <div className="chapter-chip">
            Assignment {phase.index + 1} of {phase.count} · {phase.date}
          </div>
        )}
      </header>

      <div className="meters-row solo">
        <MetersBar meters={match.meters} meta={meta} title="Your Newsroom" />
      </div>

      {standingLow && !feedback && (
        <div className="banner danger" role="alert">
          🏛️ Your paper's Standing is running very low. The chill has set in — keep telling the truth as long as you can.
        </div>
      )}
      {!standingLow && otherLow && !feedback && (
        <div className="banner danger" role="alert">
      ...
      <div className="match-body">
        <section className="action-panel" aria-live="polite">
          {feedback ? (
            ...
            <div className="waiting-panel"><div className="pulse-dot" aria-hidden="true" /><p>In the newsroom…</p></div>
          )}
        </section>

        <section className="map-panel">
          <NewsroomPanel chapterIndex={chapterIndex} />
        </section>
      </div>
    </div>
  );
...
function EventCard({ eventCard, meta, onContinue }) {
  ...
  return (
    <div className="event-card">
      <div className="event-kicker">Assignment {ch.index + 1} of {ch.count} · {ch.date}</div>
      <h2>{ch.title}</h2>
...
const VERDICT_UI = {
  right: { label: 'Good reporting', className: 'right', icon: '✓' },
