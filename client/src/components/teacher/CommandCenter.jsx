...
// the PDF → End Session (deletes everything from memory). Reconstruction
// Reporter is solo: everyone plays the same reporter and starts as soon as
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Reconstruction Reporter · everyone plays the same reporter · TEKS 7.5C, 7.20D/F, 7.16B</p>
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Reconstruction Reporter · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student opens their notebook right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        ...
        <div className="class-accuracy">
          <div className="trail-stat wide">
            <div className="trail-stat-name"><span aria-hidden="true">📰</span> The Galveston Reporter — the whole class</div>
      ...
      <footer className="app-footer">7th Grade Texas History · Reconstruction Reporter · TEKS 7.5C, 7.20D/F, 7.16B</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  const doc = new jsPDF();
  const BRAND = [35, 65, 92]; // masthead ink-blue

  doc.setFontSize(16);
  doc.text('Reconstruction Reporter — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  autoTable(doc, {
    ...
    body: [['The Galveston Reporter', String(cls.count), cls.count ? `${cls.average}%` : '—']],
    styles: { fontSize: 9 },
    headStyles: { fillColor: BRAND },
  });
  ...
  doc.text('7th Grade Texas History · Reconstruction Reporter · TEKS 7.5C, 7.20D/F, 7.16B', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`reconstruction-reporter_${roster.joinCode}_${date}.pdf`);
}
