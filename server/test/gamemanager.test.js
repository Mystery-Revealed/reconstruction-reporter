...
// Reconstruction Reporter is solo with ONE class-wide role (a single side, no
// rival), so these focus on the solo lifecycle, one class-wide accuracy group,
// and — because this game has NO early-fail — that every run plays all 12
// actions through to match:end.
import test from 'node:test';
import assert from 'node:assert/strict';
import { GameManager } from '../src/GameManager.js';
import game, { PHASES } from '../src/games/reconstructionReporter.js';
...
test('the default game is Reconstruction Reporter', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  assert.equal(manager.registry.get(joinCode).gameId, 'reconstruction-reporter');
});
...
test('solo student reports the decade and completes with 100% accuracy → The Record Stands', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  ...
  assert.equal(begin[0].payload.side, SIDE, 'the one side is the reporter');
  assert.equal(begin[0].payload.mode, 'solo');
  assert.equal(begin[0].payload.chapterCount, 6, 'six assignments');
  assert.equal(begin[0].payload.rivalMeters, null, 'no rival');
  assert.deepEqual(begin[0].payload.meters, { truth: 50, sources: 50, standing: 50 });
  ...
  assert.equal(end[0].payload.you.ending.key, 'top', 'a perfect run is The Record Stands');
  assert.equal(end[0].payload.you.failed, false);
  assert.ok(end[0].payload.you.score >= 195);
...
test('all-wrong run plays all 12 actions to the end (no early-fail) → The Story Got Away, 0%', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
...
test('class accuracy is one class-wide group (the reporter)', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
...
test('a wrong-kind move is rejected', () => {
  ...
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
  // Assignment 1 step 1 is a decision; submit a map action instead.
...
test('rejoin returns a full snapshot of the live turn', () => {
  ...
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
  playRight(manager, joinCode, res.studentId); // Assignment 1 decision 1 done; decision 2 pending
