...
app.get('/healthz', (_req, res) =>
  res.json({ ok: true, service: 'reconstruction-reporter', liveSessions: manager.registry.sessions.size }));
...
server.listen(config.port, () => {
  console.log(`[reconstruction-reporter] listening on :${config.port} — sessions live in memory only`);
});
